    ...
    methods: {
        ...
        IsInitMaintainPwd () {
            this.$api.IsInitMaintainPwd().then(res => {
                ...
                if (code === 200) {
                    ...
                    if (initData.initStatus === 0 || initData.initStatus === 1) {
        ...
        confirm (props, DefaultForm) {
            ...
            if (props === 'firstLogin') {
                ...
                this.secondLogin = false;
                let firstFormData = {
                    DefaultPassword: this.$getCode.RSAEncrypt(sha512(this.DefaultForm.DefaultPassword)),
                    InputPassword: this.DefaultForm.InputPassword
                };
            ...
            if (props === 'secondLogin') {
                ...
                this.$refs[DefaultForm].validate((valid) => {
                    if (valid) {
                        this.VerifyLogin({
                            InputPassword: sha512(this.testForm.password)
                        });
                    } else {
                        console.log('error submit!!');
                        return false;
        ...
        InitAdminPwd (formData) {
            this.$api.InitAdminPwd(formData).then(res => {
        ...
        VerifyLogin (formData) {
            this.$api.VerifyLogin(formData).then(res => {
                if (res.data.code === 200) {
                    this.secondLoading = true;
                    this.firstLogin = false;
                    this.secondLogin = false;
