    ...
    <div class="mask-layer" v-if="!tosShow">
        <aside class="tos" v-if="!isNext">
            ...
            <div class="tos-main" v-infinite-scroll="load" infinite-scroll-distance="20">
                ...
                <template v-for="pact in serviceTerms.pacts">
                    <h4 class="title">
                        {{pact.title}}
                    </h4>
            ...
            <div v-else class="bottom">{{serviceTerms.bottomTips.pleaseRead}}<i class="el-icon-bottom"></i><i class="el-icon-bottom"></i><i class="el-icon-bottom"></i></div>
        </aside>
        <aside class="tos" v-else>
            ...
            <div class="tos-main" v-infinite-scroll="privacyLoad" infinite-scroll-distance="20">
                ...
                <template v-for="pact in serviceTerms.privacyStatement.pacts">
                    <h4 class="title">
                        {{pact.title}}
                    </h4>
                    <declarationPacts :content="pact.content"></declarationPacts>
        ...
        agreeChange () {
            ...
            if (this.checkedAgree) {
                this.agreeTimeQc = setInterval(() => {
                    this.agreeTime--;
                    if (this.agreeTime == -1) {
                        window.localStorage.showDeclare = false
                        this.tosShow = true;
