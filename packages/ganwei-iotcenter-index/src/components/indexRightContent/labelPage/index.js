...
export default {
    ...
    inject: ['routesToName', 'outerLinkMap'],
    ...
    methods: {
        updateTabs () {
            this.$nextTick(() => {
                ...
                if (fullPath.toLowerCase() !== '/index') {
                    this.$nextTick(() => {
                        if (this.$route?.query?.currentPageName) {
                            this.setTableTabs(this.$route.query.currentPageName)
                            return
                        }
                        this.setTableTabs(this.routesToName[fullPath] || this.routesToName[path])
                    })
                }
            })
        },
